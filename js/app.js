...
const face = m => m.portrait ? `<div class="pt">${img(m.portrait, m.name)}</div>` : avatar(m);
const cardMember = m => `<button class="card" data-member="${esc(m.id)}">${face(m)}<h3>${esc(m.name)}</h3><p><b>${esc(m.role)}</b></p><div style="margin:8px 0">${(m.genres||[]).map(g=>`<span class="tag">${esc(g)}</span>`).join("")}</div><p>${esc(m.intro)}</p></button>`;
...
function openMember(id) {
  ...
  $("#modal-body").innerHTML = `${m.portrait ? `<div class="pt big">${img(m.portrait, m.name)}</div>` : avatar(m, true)}<h3>${esc(m.name)}</h3><p><b>${esc(m.role)}</b></p><p>${esc(m.about || m.intro)}</p>${list("得意分野", m.skills)}${list("VCA内でできること", m.canDo)}${links ? `<h4>SNS・YouTube</h4>${links}` : ""}${works ? `<h4>制作実績</h4><ul>${works}</ul>` : ""}`;
...
(async () => {
  ...
  if (D.site.logo) $(".logo").innerHTML = `<img src="${esc(D.site.logo)}" alt="VCA Virtual Creators Association" class="logo-img">`;
  if (D.site.favicon) { const l = document.createElement("link"); l.rel = "icon"; l.href = D.site.favicon; document.head.appendChild(l); }
